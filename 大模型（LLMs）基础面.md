...
2. 💡 **prefix LM 和 causal LM 区别是什么？**    
    ...
    1. Prefix LM：前缀语言模型是一种生成模型，它在生成每个词时都可以考虑之前的上下文信息（一般指Bert相关的非自回归预训练语言模型)。在生成时，模型可以看到历史以及未来的词序列。这种模型可以用于文本生成、机器翻译等任务。
    2. Causal LM：因果语言模型是一种自回归模型，它只能根据之前的文本生成后续的文本，而不能根据后续的文本生成之前的文本（一般指GPT系列的自回归预训练语言模型）。在训练时，因果语言模型的目标是预测下一个词的概率，给定之前的所有词作为上下文。这种模型可以用于文本生成、语言建模等任务。
